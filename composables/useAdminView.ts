...
import type { GameStartEvent, ImagesReadyEvent, ToSViolationEvent } from "@/types";

export function useAdminView(roomId: string) {
  ...
  const sendTosViolation = (playerId: string) => {
    const payload: ToSViolationEvent["payload"] = {
      playerId,
    };
